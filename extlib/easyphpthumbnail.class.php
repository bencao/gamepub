...
class easyphpthumbnail {
	...
	private function createemptythumbnail() {
		...
		if ($this->Percentage) {
			if ($thumbwidth>0) {$thumbwidth= $thumbwidth; } //floor($thumbwidth*($this->Percentage/100)*$this->size[0]);
			if ($thumbheight>0) {$thumbheight= $thumbheight; } //floor($thumbheight*($this->Percentage/100)*$this->size[0]);
		...
		if ($thumbheight>0) {
			...
			$this->thumb=imagecreatetruecolor(
				$this->Framewidth*$this->frame_offset+$thumbwidth+$this->shadow_offset,
				$this->Framewidth*2+ceil($this->size[1]/($this->size[0]/$thumbwidth))+$this->shadow_offset);
		} else {
...
//		else
	...
	private function savethumb() {
		...
		$cmdext = explode('.', basename($this->image));
		$filename=$cmdext[0];
		$ext = $cmdext[1];
		...
		switch($this->size[2]) {
			case 1:
				...
				imagegif($this->thumb,$this->Thumblocation.$this->Thumbprefix.$filename.$this->Thumbsubfix.'.'.$ext);
				break;
			case 2:
				//imagejpeg($this->thumb,$this->Thumblocation.$this->Thumbprefix.basename($this->image),$this->Quality);
				imagejpeg($this->thumb,$this->Thumblocation.$this->Thumbprefix.$filename.$this->Thumbsubfix.'.'.$ext,$this->Quality);
				break;
			case 3:
				//imagepng($this->thumb,$this->Thumblocation.$this->Thumbprefix.basename($this->image));
				imagepng($this->thumb,$this->Thumblocation.$this->Thumbprefix.$filename.$this->Thumbsubfix.'.'.$ext);
				break;
		}		
