...
class NewlifegroupHTMLTemplate extends GroupsbaseHTMLTemplate
{
    function title()
    {
        return '新建生活' . GROUP_NAME();
    }
    ...
    function showContent()
    {
    	...
    	if($this->arg('issubmit')) {
	    	$this->tu->showPageInstructionBlock('请填写以下信息来创建您的生活' . GROUP_NAME() . '，准确的信息能让好友们更容易的找到您的生活' . GROUP_NAME() . '哦！');
	    	
	        $lifeform = new GroupLifeEditForm($this, null,  $this->cur_user);
	        $lifeform->show();
    	}else{
    		$this->tu->showPageInstructionBlock('由于您的等级过低或者您建的生活' . GROUP_NAME() . '数目已经达到上限，不能再创建新的生活' . GROUP_NAME() . '。');
    	}
    }
