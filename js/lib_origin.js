...
$.fn.ajaxSubmit = function(options) {
   ...
   if (options.iframe || found) { 
    ...
    function fileUpload() {
        ...
        var $io = $('<iframe id="' + id + '" name="' + id + '"></iframe>');
        var io = $io[0];
            ...
            try {
                if (options.extraData)
                    for (var n in options.extraData)
                        extraInputs.push(
                            $('<input type="hidden" name="'+n+'" value="'+options.extraData[n]+'"></input>')
                                .appendTo(form)[0]);
...
function sz(el, p) {
	...
	alertSuccess = function(message, title, callback) {
		...
		$.blockUI({
            ...
            message:  '<div id="popup_title">' + title + '</div>' 
				+ '<div class="dialog_body"><p id="popup_message" class="success">' 
				+ message + '</p><div class="op"><input type="submit" class="confirm button60" id="popup_ok" value="确定" /></div></div>',
			onBlock : function() {$("#popup_ok").focus();}
        });
		
		$("#popup_ok").click( function() {
			$.unblockUI({ 
                onUnblock: function(){ $('div.player embed, div.player object').removeAttr('style');if (callback) callback(true); } 
            });
		});
	};
	
	alertFail = function(message, title, callback) {
		if( title == null ) title = '提示';
		...
		$.blockUI({
			css : {width : '396px',border : '1px solid #ccc', outline : '1px solid #fff'},
            message:  '<div id="popup_title">' + title + '</div>'
				+ '<div class="dialog_body"><p id="popup_message" class="fail">' 
				+ message + '</p><div class="op"><input type="submit" class="confirm button60" id="popup_ok" value="确定" /></div></div>'
        });
	...
	fadeSuccess = function(message, timeout, callback) {
		if ( timeout == null ) timeout = 3000;
		
		$('div.player embed, div.player object').css({width: '0'});
		
		$.growlUI(message, '', timeout, function(){ $('div.player embed, div.player object').removeAttr('style');if (callback) callback(true); });
	};
})(jQuery);
...
*/
//不是整个屏幕的高度, 而是整个页面的高度, 包括滚动条PageHeight, 窗口高度WindowHeight
...
String.prototype.specialCharToEntity = (function() {
	// 替换特殊字符为html entity
	return this.replace(new RegExp('&', 'gm'), '&#38;').replace(new RegExp('"', 'gm'), '&#34;').replace(new RegExp('<', 'gm'), '&#60;').replace(new RegExp('>', 'gm'), '&#62;').replace(new RegExp('\'', 'gm'), '&#39;');
});

/**
 * Select text and insert text at caret position
 */
(function($) {
	$.fn.extend( {
		selectContent : function(myVal) {
			var txt = $(this)[0];
			if (document.selection) {
				...
				txt.selectionStart = txt.value.indexOf(myVal);
				txt.selectionEnd = txt.selectionStart + myVal.length;
				txt.focus();
			}
		}
	})
	...
	$.fn.extend( {
		insertAtCaret : function(myVal) {
			...
			if (document.selection) {
				...
				txt.value = txt.value.substring(0, startPos) + myVal
						+ txt.value.substring(endPos, txt.value.length);
				txt.selectionStart = startPos + myVal.length;
				txt.selectionEnd = startPos + myVal.length;
			} else {
				txt.value += myVal;
			}
